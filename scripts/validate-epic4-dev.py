...
SCAFFOLD = ROOT / "gitops" / "epic4" / "base" / "epic4-offline-scaffold.yaml"
DEPLOYMENTS = [
    ROOT / "gitops" / "epic4" / "overlays" / overlay / "kustomization.yaml"
...
REQUIRED_TEXT = [
    ...
    "name: epic4-observability-pipeline",
    "vmstorage",
    "vminsert",
    "vmselect",
    "vmlog",
...
def validate() -> list[str]:
    ...
    for item in REQUIRED_TEXT:
        if item not in manifest:
            failures.append(f"epic4-offline-scaffold.yaml missing {item}")

    for overlay in DEPLOYMENTS:
        overlay_text = overlay.read_text(encoding="utf-8")
        if "../../base/epic4-offline-scaffold.yaml" not in overlay_text:
            failures.append(f"{overlay.relative_to(ROOT)} missing epic4 base resource")

    return failures


def main() -> int:
    parser = argparse.ArgumentParser(description="Validate Epic 4 offline GitOps scaffold")
