...
- [x] [Review][Decision] Standalone broker memory budget — `PULSAR_MEM=-Xms512m -Xmx1024m` covers only the broker JVM; in-process BookKeeper (direct-memory caches) shares a single 2048Mi container limit. **RESOLVED 2026-08-29 (D2 answered "accept for dev load"):** broker ran healthy under smoke load (produce/consume ×5, 12 partitions) within the current budget; no OOM or latency anomaly observed. Interim posture stands; revisit `BOOKKEEPER_MEM`/direct-memory caps if production load lands.
- [x] [Review][Decision] Spec record contradiction — story KNOWN-GAP says Pulsar "cannot be hosted / was uninstalled", but the diff ships it and the cluster runs it healthy. **RESOLVED 2026-08-29 (D3 answered: amend record — no new milestone story):** frontmatter KNOWN-GAP + Completion Notes amended with "SUPERSEDED 2026-08-29" + live evidence (pulsar Synced Healthy, broker 4.2.4 + operator v0.21.0, tenant/ns/topics provisioned, smoke-tested). Pulsar milestone fully contained in 11-5.
