...
- **Follow-up 2026-08-30 (route realignment, Pulsar gap cleanup):** base manifests aligned to the committed rendered state that `325c8cb` had written to overlays only. Dropped the `http-redirect` listener (port 80) and the gRPC/MQTT ingestion routes from base; removed the `hpdc-telemetry-grpc-api-key-authn` SecurityPolicy; re-based `/data` onto the real `couchdb-entity-store` Service (entity-store, ReferenceGrant) and `/telemetry` onto `pulsar-standalone` (pulsar, ReferenceGrant); pinning EG-mutated fields (`addresses`, `certificateRefs.group`, parentRef/backendRef `group`/`kind`) so ArgoCD self-heal sees no drift. Fixed two further base/rendered divergences found during re-render: base RBAC ported from the v1.9-aligned set (`5170d24` rendered-only) and EnvoyProxy spec neutralized to `{}` (`e69e22f` rendered-only). `tests/test_install_telemetry_ingestion_dev.py` + `test_p0_route_table_audit.py` updated; gRPC/mqtt route absence asserted. `test_p0_security_network.py::test_p0_016` remains a pre-existing failure (client reads singular `targetRef`; policies migrated to plural `targetRefs`, unchanged at HEAD).
