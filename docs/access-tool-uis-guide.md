...
- The HPDC dev cluster is running (Talos on Docker or QEMU).
- The Envoy Gateway (`hpdc-edge`) is deployed with a static self-signed TLS wildcard certificate for `*.hpdc.local` (generated by `scripts/gitops/gen-edge-cert.py`).
- Cilium L2 load balancing assigns the Envoy Gateway a LoadBalancer address from the pool defined by `HPDC_CILIUM_LB_POOL_RANGE` in `.env` (default `172.18.0.0/24`).

> **Docker cluster note:** On Docker-based dev clusters, Cilium L2 announcement does not work
> at the host level because Docker bridge networking does not support ARP/NDP announcements
> for external IPs. Use `kubectl port-forward` as a workaround:
> ```bash
> kubectl port-forward -n envoy-gateway-system svc/envoy-<gateway-name>-<hash> 8443:443
> ```
> On QEMU VM or bare-metal clusters, L2 announcement works natively and the LoadBalancer IP
> is routable from the host.
...
| Envoy Gateway / TLS | `gitops/envoy-gateway/base/envoy-gateway.yaml`, `scripts/gitops/gen-edge-cert.py` |
