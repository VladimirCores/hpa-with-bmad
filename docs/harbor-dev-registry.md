...
```python
./scripts/install-harbor-dev.py --offline --apply
```

Validation mode:

```python
python3 scripts/install_harbor_dev.py --check
```
...
python3 -m py_compile scripts/install-harbor-dev.py scripts/install_harbor_dev.py tests/test_install_harbor_dev.py
```
