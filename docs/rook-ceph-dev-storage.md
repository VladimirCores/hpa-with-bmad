...
```python
python3 scripts/startup.dev.py --offline --dry-run --step 05-install-rook-ceph-dev.py
```

To apply the GitOps overlay to the Talos cluster after Cilium is installed:

```python
python3 scripts/startup.dev.py --offline --apply --step 05-install-rook-ceph-dev.py
```

Validation mode:

```python
python3 scripts/startup.dev.py --offline --check --step 05-install-rook-ceph-dev.py
```
